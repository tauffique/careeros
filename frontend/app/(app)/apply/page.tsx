...
export default function ApplyPage() {
  ...
  async function runAtsBefore() {
    ...
    try {
      // First create the application via match (which saves it)
      const matchData = await authFetch("/applications/match", {
        job_description: jd, company, role, n_results: 3,
      });
      setApplicationId(matchData.application_id);
      setMatchResult(matchData);

      // Then score ATS before
      const atsData = await authFetch("/applications/ats-score", {
        application_id: matchData.application_id, stage: "before",
      });
  ...
  // Step 4 — ATS score after
  async function runAtsAfter() {
    setLoading(true); setError("");
    try {
      const data = await authFetch("/applications/ats-score", {
        application_id: applicationId, stage: "after",
      });
      setAtsAfter(data);
      setStage("ats_after");
    } catch (e: any) { setError(e.message); }
  ...
  function reset() {
    setJd(""); setCompany(""); setRole(""); setLanguage("English");
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "28px" }}>
        <div>
          ...
          <p style={{ color: C.mid, fontSize: "13px", margin: "4px 0 0" }}>Paste a JD — CareerOS retrieves your best projects and generates tailored documents</p>
        </div>
        {stage !== "input" && (
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "0", marginBottom: "28px" }}>
        {[
          ...
          { key: "ats_before",label: "ATS Before" },
          { key: "matched",   label: "RAG Match" },
          { key: "generated", label: "Generate" },
          { key: "ats_after", label: "ATS After" },
        ].map(({ key, label }, i) => {
            ...
            <div key={key} style={{ display: "flex", alignItems: "center", flex: i < 4 ? 1 : "none" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                <div style={{
                  ...
                  color: done || active ? "#fff" : C.light, flexShrink: 0,
                }}>
                  {done ? "✓" : i + 1}
                </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: stage === "input" ? "1fr" : "1fr 340px", gap: "20px", alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: "12px", padding: "24px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
            <SectionHeader title="Job Description" subtitle="Paste the full JD — enter company and role manually" />
  ...
  <select style={{ ...inp, opacity: stage !== "input" ? 0.6 : 1, maxWidth: "200px" }} value={language} onChange={e => setLanguage(e.target.value)} disabled={stage !== "input"}>
    {LANGUAGES.map(l => <option key={l}>{l}</option>)}
          ...
          {/* Step 2 — RAG Match results */}
          {matchResult && (
            <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: "12px", padding: "24px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
              <SectionHeader title="RAG Match Results" subtitle={`Top ${matchResult.retrieved_projects?.length} projects retrieved from your knowledge base`} />
                ...
                {matchResult.retrieved_projects?.map((p: any, i: number) => (
                  <div key={p.id} style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: "8px", padding: "12px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      ...
                      <span style={{ fontSize: "11px", fontWeight: "700", color: C.green, background: C.greenLight, padding: "1px 7px", borderRadius: "20px" }}>
          ...
          {(cvLatex || clLatex) && (
            <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: "12px", padding: "24px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px" }}>
                ...
                <button
                  onClick={() => downloadTex(activeTab === "cv" ? cvLatex : clLatex, activeTab === "cv" ? `CV_${company || "careeros"}` : `CoverLetter_${company || "careeros"}`)}
                  style={{ fontSize: "12px", padding: "5px 12px", background: C.indigo, color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer", fontWeight: "600" }}>
                  ⬇ Download .tex
                </button>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "12px" }}>
                <p style={{ fontSize: "11px", color: C.light, margin: 0 }}>Copy → Overleaf → New Project → Blank → paste into main.tex → Compile</p>
                {!atsAfter && (
                  <button onClick={runAtsAfter} disabled={loading}
                    style={{ padding: "7px 16px", background: C.indigo, color: "#fff", border: "none", borderRadius: "7px", fontSize: "12px", fontWeight: "700", cursor: loading ? "not-allowed" : "pointer" }}>
              ...
              <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: "12px", padding: "20px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                ...
                {atsBefore.matched_keywords?.length > 0 && (
                  <div style={{ marginBottom: "12px" }}>
                    <div style={{ fontSize: "11px", fontWeight: "600", color: C.green, marginBottom: "6px" }}>✓ Matched keywords</div>
                ...
                {atsBefore.missing_keywords?.length > 0 && (
                  <div>
                    <div style={{ fontSize: "11px", fontWeight: "600", color: C.red, marginBottom: "6px" }}>✗ Missing keywords</div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
              ...
              <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: "12px", padding: "20px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                ...
                {scoreDelta !== null && (
                  <div style={{ background: scoreDelta > 0 ? C.greenLight : C.redLight, border: `1px solid ${scoreDelta > 0 ? "#86EFAC" : "#FCA5A5"}`, borderRadius: "8px", padding: "10px 14px", marginBottom: "12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontSize: "12px", color: scoreDelta > 0 ? C.green : C.red, fontWeight: "600" }}>
                      {scoreDelta > 0 ? "↑" : "↓"} {Math.abs(scoreDelta)} point improvement
                    </span>
                    <span style={{ fontSize: "18px", fontWeight: "800", color: scoreDelta > 0 ? C.green : C.red }}>
