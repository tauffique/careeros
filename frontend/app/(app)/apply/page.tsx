...
function loadSession(){try{const s=sessionStorage.getItem("apply_state");return s?JSON.parse(s):null;}catch{return null;}}
function saveSession(state:any){try{sessionStorage.setItem("apply_state",JSON.stringify(state));}catch{}}
...
export default function ApplyPage(){
  ...
  useEffect(()=>{saveSession({jd,language,stage,applicationId,atsBefore,matchResult,cvLatex,clLatex,atsAfter});},[jd,language,stage,applicationId,atsBefore,matchResult,cvLatex,clLatex,atsAfter]);
  ...
  async function downloadPDF(latex:string,filename:string){
    ...
    try{
      ...
      formData.append("file",new Blob([latex],{type:"text/plain"}),"document.tex");
      const res=await fetch("https://latexonline.cc/compile",{method:"POST",body:formData});
      ...
      const url=URL.createObjectURL(blob);
      const a=document.createElement("a");a.href=url;a.download=`${filename}.tex`;a.click();
      URL.revokeObjectURL(url);
      setError("PDF compilation failed — downloaded .tex instead. Open in Overleaf to compile.");
          ...
          {(cvLatex||clLatex)&&(
            <div style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:"12px",padding:"24px",boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
              <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"16px"}}>
                ...
                <button onClick={()=>downloadPDF(activeTab==="cv"?cvLatex:clLatex,activeTab==="cv"?"CV_karriereos":"CoverLetter_karriereos")} disabled={pdfLoading}
